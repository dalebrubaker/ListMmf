...
public class SmallestInt64Tests : IDisposable
{
    ...
    {
        using var smallest = new SmallestInt64ListMmf(DataType.Empty, TestPath);
    ...
    public void UpgradeSByteToInt32_Test()
    {
        using (var smallest = new SmallestInt64ListMmf(DataType.SByte, TestPath))
        {
            ...
            smallest.WidthBits.Should().Be(8, "sbyte");
            smallest.Add(1);
        } // must Dispose to close the file before upgrading

        var isUpgraded = SmallestInt64ListMmf.UpgradeIfRequired(int.MinValue, int.MaxValue, TestPath);
        isUpgraded.Should().BeTrue();
        var (version, dataType, count) = UtilsListMmf.GetHeaderInfo(TestPath);
        version.Should().Be(0);
        count.Should().Be(1, "Only one value moved.");
        dataType.Should().Be(DataType.Int32);
    }
    ...
    public void UpgradeBitArrayToInt32_Test()
    {
        ...
        var isUpgraded = SmallestInt64ListMmf.UpgradeIfRequired(sbyte.MinValue, sbyte.MaxValue, TestPath);
        isUpgraded.Should().BeTrue();
        var (version, dataType, count) = UtilsListMmf.GetHeaderInfo(TestPath);
        count.Should().Be(1, "Only one value moved.");
        dataType.Should().Be(DataType.SByte);
    ...
    [Fact]
    public void UpgradeInt16ToInt32_Timing_Test()
    {
        const int CountForTest = ushort.MaxValue + 1;
        ...
        var elapsedMsecWrite = sw.ElapsedMilliseconds;
        sw.Restart();
        File.Exists(path).Should().BeTrue();
        var isUpgraded = SmallestInt64ListMmf.UpgradeIfRequired(int.MinValue, int.MaxValue, TestPath);
        var elapsedMsec = sw.ElapsedMilliseconds;
        isUpgraded.Should().BeTrue();
        var (version, dataType, count) = UtilsListMmf.GetHeaderInfo(TestPath);
        count.Should().Be(CountForTest, "Only one value moved.");
        dataType.Should().Be(DataType.Int32);
    }
