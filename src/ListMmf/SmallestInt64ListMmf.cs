    ...
    public class SmallestInt64ListMmf : IListMmf<long>, IReadOnlyList64Mmf<long>
    {
        ...
        public void Add(long value)
        {
            lock (_lock)
            {
                ...
                //s_logger.ConditionalDebug($"Add {value} in {this}");
        ...
        public void Dispose()
        {
            lock (_lock)
            {
                //s_logger.ConditionalDebug($"Disposing _underlying={_underlying} in {this}");
                _isDisposed = true;
                _underlying?.Dispose();
                _underlying = null;
            }
        ...
        private void UpgradeUnderlying(long minValueRequired, long maxValueRequired)
        {
            lock (_lock)
            {
                ...
                //s_logger.ConditionalDebug($"Disposing  _underlying for upgrade in {this}");
                _underlying.Dispose();
                //s_logger.ConditionalDebug($"Setting _underlying to null in {this}");
                _underlying = null;
                ...
                //s_logger.ConditionalDebug($"Upgraded underlying to {_underlying} in {this}");
            }
        }
        ...
        private static void Upgrade(DataType dataTypeNew, string path, DataType dataTypeExisting, string name, IProgressReport progress)
        {
            try
            {
                ...
                using (var source = new SmallestInt64ListMmf(dataTypeExisting, tmpPath, 0L, MemoryMappedFileAccess.Read))
                {
                    ...
                    s_logger.ConditionalDebug(message);
        ...
        private class Underlying : IListMmf<long>, IReadOnlyList64Mmf<long>
        {
            ...
            public void Dispose()
            {
                //s_logger.ConditionalDebug($"Disposing _underlying and setting to to null in {this}");
                _iListMmf?.Dispose();
                _iListMmf = null;
            }
