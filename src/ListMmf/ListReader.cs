...
{
    [Serializable]
    public class ListReader<T> : IReadOnlyList64<T>
    {
        ...
        /// <param name="count">The number of elements of the view of array included in this list. Specify 0 (the default) to include all remaining elements in the array starting at beginIndex</param>
        public ListReader(IList<T> array, int beginIndex = 0, int count = 0)
        {
            _array = array;
        ...
        public static IReadOnlyList64<T> Empty => s_empty ??= new ListReader<T>(new List<T>());
        ...
        {
            private readonly ListReader<T> _list;
            private long _index;

            internal Enumerator(ListReader<T> list)
            {
                _list = list;
                _index = 0;
